...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "convertToMarkdown") {
    (async () => {
      try {
        const result = await convertCurrentPageToMarkdown();
    ...
    try {
      ...
      // Try deepwiki.com sidebar selector first
      let sidebarLinkElements = Array.from(document.querySelectorAll('.border-r-border ul li a'));
      ...
      if (sidebarLinkElements.length > 0) {
        ...
        // Devin wiki structure: sidebar uses button elements without href.
        // First expand any collapsed/nested sections so every page is captured.
        await expandAllDevinSidebarSections();

        // Buttons are inside [data-slot="sidebar-content"] area
        const sidebarContent = document.querySelector('[data-slot="sidebar-content"]') ||
                               document.querySelector('[data-slot="sidebar-content-wrapper"]');
        const buttons = sidebarContent
          ? Array.from(sidebarContent.querySelectorAll('[data-slot="sidebar-menu-item"] button[aria-label]'))
          : [];
        
        // Build base wiki path from current URL (strip trailing page slug if present)
        const currentHref = window.location.href;
        const urlObj = new URL(currentHref);
        const queryString = urlObj.search; // e.g. ?branch=main
        // The wiki root is /org/{orgId}/wiki/{owner}/{repo}
        // Strip any extra path segments beyond the repo to get the root
        const pathParts = urlObj.pathname.split('/').filter(Boolean);
        // Find "wiki" segment and take up to owner/repo (4 segments after wiki prefix)
        const wikiIndex = pathParts.indexOf('wiki');
        const wikiRootPath = wikiIndex !== -1
          ? '/' + pathParts.slice(0, wikiIndex + 3).join('/') // /org/{orgId}/wiki/{owner}/{repo}
          : urlObj.pathname;
        const wikiRootUrl = urlObj.origin + wikiRootPath;
        
        // De-duplicate by aria-label in case expanding revealed repeats.
        ...
        pages = buttons
          ...
          .map((btn, index) => {
            ...
            const slug = label.toLowerCase()
              ...
              .replace(/[^a-z0-9]+/g, '-')
              .replace(/^-|-$/g, '');
            const parentButton = btn.closest('[data-slot="sidebar-menu-button"]');
            const isActive = parentButton?.getAttribute('data-active') === 'true';
            return {
      ...
      // Whether this page is a Devin SPA wiki (sidebar uses buttons, not links).
      // In that case the popup should navigate by clicking instead of reloading.
      const isDevin = sidebarLinkElements.length === 0;
    ...
    sendResponse({ received: true });
  } else if (request.action === "clickDevinNavAndConvert") {
    // Robust Devin (SPA) batch step: click the real sidebar button, wait for the
    // content to swap in-place (no reload / no URL guessing), then convert.
    (async () => {
      try {
        const previousTitle =
          document.querySelector('h1[data-header="true"]')?.textContent?.trim() || "";

        // Make sure nested sections are expanded so the target is reachable.
        await expandAllDevinSidebarSections();

        const buttons = getDevinNavButtons();
        let target = null;
        if (typeof request.label === "string") {
          target = buttons.find(
            (b) => b.getAttribute("aria-label")?.trim() === request.label
          );
        }
        if (!target && typeof request.index === "number") {
          target = buttons[request.index];
        }
        if (!target) {
          sendResponse({
            success: false,
            error: "Sidebar item not found: " + (request.label ?? request.index)
          });
          return;
        }
        ...
        if (alreadyActive) {
          // Already on this page; just let it settle.
          await new Promise((r) => setTimeout(r, 300));
        } else {
          await waitForDevinContentUpdate(previousTitle, 12000);
        }

        // Skip conversion if this was only a restore-to-original navigation.
        if (request.convert === false) {
        ...
        console.error("Error in clickDevinNavAndConvert:", error);
        sendResponse({ success: false, error: error.message });
      }
    })();
  }
...
// Radix/shadcn collapsibles, whose toggles are marked with aria-expanded="false"
// or data-state="closed" -- so we only click real expand toggles, never plain
// page links. Loops until nothing collapsed remains (with a hard round cap),
// which also handles nested collapsibles revealed by an earlier expansion.
async function expandAllDevinSidebarSections(maxRounds = 12) {
  const sidebar =
  ...
  for (let round = 0; round < maxRounds; round++) {
    const collapsed = Array.from(
      sidebar.querySelectorAll('[aria-expanded="false"], [data-state="closed"]')
    ).filter((el) => {
      if (!(el instanceof HTMLElement)) return false;
      // Skip hidden toggles (e.g. inside a not-yet-revealed parent).
    ...
    // Let newly revealed sub-items render before the next round.
...
// Collect the Devin wiki sidebar navigation buttons (SPA, no href links).
function getDevinNavButtons() {
  const sidebarContent =
    document.querySelector('[data-slot="sidebar-content"]') ||
    document.querySelector('[data-slot="sidebar-content-wrapper"]');
  if (!sidebarContent) return [];
  return Array.from(
    sidebarContent.querySelectorAll('[data-slot="sidebar-menu-item"] button[aria-label]')
  ).filter((btn) => btn.getAttribute("aria-label")?.trim());
}

// Wait for the Devin SPA main content to swap after a sidebar click. Resolves
// when the article header changes, or after a timeout (so conversion still runs).
function waitForDevinContentUpdate(previousTitle, timeout = 12000) {
  return new Promise((resolve) => {
    const start = Date.now();
    const check = () => {
      const h1 =
        document.querySelector('h1[data-header="true"]')?.textContent?.trim() || "";
      if (h1 && h1 !== previousTitle) {
        // Give the content a moment to finish rendering diagrams/tables.
...
// Find the element that actually scrolls the article content. Devin may put the
// scrollbar on an inner container rather than the window.
function getContentScrollContainer() {
  const content =
    document.querySelector('[class~="prose-main"]') ||
    document.querySelector(".prose") ||
    document.querySelector('h1[data-header="true"]');
  let el = content;
...
// Scroll through the page to force lazy-rendered content (Mermaid diagrams,
// images, streamed sections) to load, waiting until the height stabilizes.
// Then return to the top. Works for both window-level and inner-container scroll.
async function ensureContentFullyLoaded(maxRounds = 60, settleMs = 350) {
  const el = getContentScrollContainer();
  const isWindow =
    el === document.scrollingElement ||
    el === document.documentElement ||
    el === document.body;

  const getScrollHeight = () => el.scrollHeight;
  const getClientHeight = () => (isWindow ? window.innerHeight : el.clientHeight);
  const getScrollTop = () =>
    isWindow ? window.scrollY || document.documentElement.scrollTop : el.scrollTop;
  const setScrollTop = (t) =>
    isWindow ? window.scrollTo(0, t) : (el.scrollTop = t);
  ...
  for (let i = 0; i < maxRounds; i++) {
    ...
    const atBottom = getScrollTop() + getClientHeight() >= h - 8;
    if (h === prevHeight) {
      unchanged++;
      // Height stable and we've reached the end: nothing more to load.
      if (unchanged >= 2 && atBottom) break;
    } else {
      unchanged = 0;
    }
    prevHeight = h;
  ...
  // Return to the top so conversion and the user view start clean.
  setScrollTop(0);
  await new Promise((r) => setTimeout(r, 150));
}

// Convert the currently rendered page to Markdown. Shared by the single-page
// and batch (Devin in-page navigation) flows. Scrolls first to force any
// lazy-loaded content to render before reading the DOM.
async function convertCurrentPageToMarkdown() {
  await ensureContentFullyLoaded();
  ...
  // Get article title (prefer Devin DeepWiki header if present)
  const title =
    document.querySelector('h1[data-header="true"]')?.textContent?.trim() ||
    document
  ...
  // Get article content container (support both DeepWiki and Devin structures).
  // Devin renders content inside `.prose-main` (no bare `.prose` class).
  ...
  let markdown = ``;
  const markdownTitle = title.replace(/\s+/g, "-");
  ...
  return { markdown, markdownTitle, headTitle: formattedHeadTitle };
}
